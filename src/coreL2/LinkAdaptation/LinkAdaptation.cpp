...
uint8_t
LinkAdaptation::getSinrConvertToCqiUplink(
    float sinr)     //Signal to Interference plus Noise Ratio informed by PHY
{   
    uint8_t cqiUplink;      //Channel Quality Information for Uplink
    //PROVISIONAL: do some calculations/Look tables here
    cqiUplink = 10;
    return AdaptiveModulationCoding::getCqiUplinkConvertToUplinkMcs(cqiUplink);
}